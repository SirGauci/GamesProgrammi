...
public:
	...
	*********************************************************************************************/
	bool drop(const T &element){
		...
		while(N != last){
			...
			/*
			- Bug: Upon dropping data, it would drop all entries and the program did not continue
			- Fix: Branching statement to iterate through the list
			*/
			else
			{
				N = N->getNext(); 
	...
	*********************************************************************************************/
	void print (void) {
		/*
		- Error Code: C4700: unitialised local variable 'N' used
		- Initialise N to first  to iterate through the entire list
		*/
		Node *N = first;
		while (N != last) {
			cout << N->getValue() << endl;
			/*
			- Bug: Does not go through the entire list but prints 'three' repeatedly 
			- Fix: Assign N to the next value
			*/
			N = N->getNext(); 
			/*
			- Bug: Did not print the final number of the list
			- Fix: When N is equal to last, print N before the loop breaks
			*/
			if (N == last)
			{
				cout << N->getValue() << endl;
...
int main(int argc, char* argv[]){
	...
	string test;
	/*
	- Error Code: C4700: unitialised local variable 'L' used
	- Initialised L using 'new' keyword as pointers are being used
	*/
	...
	//add the last number
	/*
	- Bug: Added s6 to the start of the list
	- Change prepend with append
	*/
	L->append(s6);	
	L->print();		//Done!
	cin >> test;
